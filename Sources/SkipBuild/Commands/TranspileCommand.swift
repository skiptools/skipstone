    ...
    private func transpileThrows(root rootPath: AbsolutePath, project projectFolderPath: AbsolutePath, module moduleRootPath: AbsolutePath, skip skipFolderPath: AbsolutePath, output outputFolderPath: AbsolutePath, fs: FileSystem, with out: MessageQueue) async throws {
        ...
        func saveSkipBridgeCode() throws {
            ...
            if let skipBridgeOutput = transpileOptions.skipBridgeOutput {
                ...
                var swiftBridgeFileNameTranspilationMap = skipBridgeTranspilations.reduce(into: Dictionary<String, Transpilation>()) { result, transpilation in
                ...
                for swiftSourceFile in sourceURLs.filter({ $0.pathExtension == "swift"}) {
                    ...
                    if let bridgeTranspilation = swiftBridgeFileNameTranspilationMap.removeValue(forKey: swiftBridgeFileName) {
                ...
                // write the remaining generated bridge code, which may include support files
                for (swiftBridgeFileName, bridgeTranspilation) in swiftBridgeFileNameTranspilationMap {
                    let outputPath = skipBridgeOutputFolder.appending(components: [swiftBridgeFileName])
                    let contents = bridgeTranspilation.output.content
                    try writeChanges(tag: "skipbridge", to: outputPath, contents: contents.utf8Data, readOnly: true)
                }
